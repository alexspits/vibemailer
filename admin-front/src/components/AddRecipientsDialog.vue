...
watch(isOpen, (opened) => {
  // Сбрасываем и при открытии тоже: если диалог закрыли, пока летел разбор, ответ
  // пришёл уже на закрытый и перевёл его на шаг предпросмотра. Следующее открытие
  // показывало чужой разбор с активной кнопкой, а отправился бы пустой список.
  if (opened) {
    isInputStep.value = true;
